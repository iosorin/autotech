            ...
            <div className="flex flex-col md:flex-row gap-6">
              ...
              <div className="flex flex-col items-center md:items-start flex-shrink-0 w-full md:w-1/5">
                <div className="size-20 rounded-full bg-muted flex items-center justify-center text-2xl font-bold text-muted-foreground mb-3 overflow-hidden">
                  {item.avatar ? (
                    <Image src={item.avatar} alt={item.name} width={80} height={80} className="object-cover" title={item.name + " - Аватар"} />
                  ) : (
                    item.name.charAt(0)
                  )}
                </div>
                <div className="h4 font-bold text-foreground text-sm text-center md:text-left leading-snug">
                  {item.name}
                </div>
              ...
              <div className="flex-1">
                {item.logo ? (
                  <div className="mb-4">
                    <Image src={item.logo} alt={item.company} width={80} height={32} className="h-8 w-auto" title={item.company + " - Логотип"} />
                  </div>
                ) : (
                  <div className="mb-4">
          ...
          <div className="rounded-2xl p-6 bg-white h-full shadow-sm">
            <div className="flex items-start gap-4 mb-4">
              <div className="size-16 rounded-full bg-muted flex items-center justify-center text-lg font-bold text-muted-foreground flex-shrink-0 overflow-hidden">
                {next.avatar ? (
                  <Image src={next.avatar} alt={next.name} width={64} height={64} className="object-cover" title={next.name + " - Аватар"} />
                ) : (
                  next.name.charAt(0)
                )}
              </div>
              <div>
                <div className="h4 font-bold text-foreground text-sm leading-snug">{next.name}</div>
