...
export const Block = ({
    ...
    const renderGroup = (group: Group) => (
        <div key={group.title} className="pb-6 border-b border-border last:border-b-0">
            <div className="h4 mb-3 text-foreground">{group.title}</div>
    ...
    return (
        <div className={cn("flex flex-col lg:flex-row justify-between items-center gap-8 lg:gap-20", className, reverse ? "lg:flex-row-reverse" : "")}>
            ...
            {image &&
                <Enter variant="fade-left" delay={200} duration={700} className={cn("order-2 w-full w-auto flex-[0_0_35%]", image.className)}>
                    <Image
                        src={image.href}
                        alt={image.alt}
                        width={500}
